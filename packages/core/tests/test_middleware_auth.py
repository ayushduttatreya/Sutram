...
from sutram_core.middleware.auth import decode_jwt, AuthError
...
def make_token(tenant_id: str, user_id: str, secret: str = "test-secret") -> str:
    ...
    return jwt.encode(
        {"tenant_id": tenant_id, "user_id": user_id, "sub": user_id, "exp": int(time.time()) + 3600},
        secret,
        algorithm="HS256",
    )
...
def test_decode_valid_jwt():
    ...
    with patch("sutram_core.middleware.auth.get_jwt_secret", return_value="test-secret"):
        with patch("sutram_core.middleware.auth.get_jwt_algorithm", return_value="HS256"):
...
def test_decode_jwt_wrong_secret_raises():
    ...
    with patch("sutram_core.middleware.auth.get_jwt_secret", return_value="correct-secret"):
        with patch("sutram_core.middleware.auth.get_jwt_algorithm", return_value="HS256"):
            with pytest.raises(AuthError):
                decode_jwt(token)
...
def test_decode_jwt_missing_exp_raises():
    ...
    with patch("sutram_core.middleware.auth.get_jwt_secret", return_value="test-secret"):
        with patch("sutram_core.middleware.auth.get_jwt_algorithm", return_value="HS256"):
            with pytest.raises(AuthError, match="Missing exp"):
                decode_jwt(token)
