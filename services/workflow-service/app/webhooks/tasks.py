...
async def _deliver(delivery_id: str, attempt: int) -> None:
    ...
    async with get_db_session_context() as session:
        ...
        secret = decrypt_secret(subscription.secret_encrypted, settings.webhook_secret_encryption_key)
        ...
        if 200 <= status_code < 300:
            ...
            delivery.next_retry_at = datetime.now(timezone.utc) + timedelta(seconds=RETRY_DELAYS[attempt])
