...
def upgrade() -> None:
    ...
    op.execute("ALTER TABLE memory_items ALTER COLUMN embedding TYPE vector(1536) USING embedding::vector(1536)")

    op.create_index("idx_memory_items_tenant_id", "memory_items", ["tenant_id"])
    op.create_index(
        "idx_memory_items_tenant_compressed", "memory_items", ["tenant_id", "compressed"],
        postgresql_where=sa.text("compressed = false"),
    )
    op.create_index("idx_memory_items_tenant_type", "memory_items", ["tenant_id", "memory_type"])
    op.create_index(
        "idx_memory_items_tenant_created", "memory_items", ["tenant_id", "created_at"],
        postgresql_where=sa.text("compressed = false"),
    )
    op.execute(
    ...
    op.create_table(
        ...
        sa.Column("original_ids", postgresql.ARRAY(postgresql.UUID(as_uuid=True)), nullable=False, server_default="'{}'"),
        sa.Column("embedding", sa.Text, nullable=False),
        sa.Column("embedding_model", sa.Text, nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()"), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()"), nullable=False),
    )
    op.execute("ALTER TABLE memory_summaries ALTER COLUMN embedding TYPE vector(1536) USING embedding::vector(1536)")
